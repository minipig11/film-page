...
import { getLocalOssVideoUrl, getSignedOssVideoUrl, getCourseAndChapterTitle } from  '../utils/ossServer';
import { OSS_BASE_URL,PICTURE_BASE_URL } from '@/config/index.js' // 导入公共配置的URL

let route = useRoute();
let {courseId , chapterId, isTrial } = route.query;

const loadVideo = async () => {

  // 根据 isTrial 加载视频 URL
  if (isTrial === 'local') {
    // options.src = `${PICTURE_BASE_URL}${courseId}/${chapterId}.mp4`;
    options.src = await getLocalOssVideoUrl(courseId, chapterId);
  } else if (isTrial === 'oss') {
    options.src = await getSignedOssVideoUrl(courseId, chapterId);
  } else if (isTrial === 'xhs') {
    options.src = getPlayUrl(courseId, chapterId);
  }
};
...
onBeforeMount(()=>{
  // 设置视频标题
  options.title = getCourseAndChapterTitle(courseId, chapterId);
  loadVideo(); // 调用异步函数
  console.log('开始学习:', options.src);
});
